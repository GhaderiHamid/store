                    ...
                    @foreach ($products as $item)
                        @if (isset($reservations[$item->id]))
                            ...
                            <div class="w-100 d-flex align-items-center mt-2 product-border p-2"
                                data-product-wrapper="{{ $item->id }}">
                                <img class="card-img-top w-25 rounded" src="/{{ $item['image_path'] }}"
                                    alt="{{ $item->name }}">
                                            ...
                                            <div class="form-group d-flex justify-content-center row align-items-center">
                                                <div class="d-flex flex-column align-items-center justify-content-center">
                                                    ...
                                                    <div class="text-warning text-center mt-2">
                                                        رزرو این محصول
                                                        <span class="reservation-timer" data-seconds="{{ $remaining }}"
                        ...
                        <form action="{{ route('payment.process') }}" method="POST">
                            ...
                            <input type="hidden" name="data" id="data-input"
                                value="{{ json_encode([
                                    'subtotal' => $total,
                                    'user_id' => auth('web')->id(),
                                    'products' => $products->filter(fn($item) => isset($reservations[$item->id]))->map(function ($item) use ($cart) {
